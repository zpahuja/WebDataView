...
xSortedChildren = children.sort(sortGlobalBlockIndicesByAttr('-vips-startY'));
// Group children by same dimensions
xGroups = groupSortedIndices(xSortedChildren);

// sort children by start Y
ySortedChildren = children.sort(sortGlobalBlockIndicesByAttr('-vips-startY'));
// Group children by same dimensions
yGroups = groupSortedIndices(ySortedChildren);

// Get cluster id of elements in each group. Find majority of each group and store in dictionary from id to group_index in groups. Create new groups and color code them
clusterId2GroupIndex = {};
clusterIdKeys = [];
groups = xGroups;
for (var i = 0; i < groups.length; i++) {
    var group = groups[i];
    var clusterIds = new Array(group.length);

    for (var j = 0; j < group.length; j++) {
        clusterIds[j] = id2cluster[group[j]];
    }

    var majorityClusterId = mode(clusterIds);
    var key = majorityClusterId.toString();
    clusterId2GroupIndex[key] = i;
    clusterIdKeys.push(key);
}

// Form new groups
var mergedGroups = [];
var tempGroup = [];
for (var key in clusterIdKeys) {
    var groupsToMerge = clusterId2GroupIndex.key;
    if (groupsToMerge) {
        for (var i = 0; i < groupsToMerge.length; i++) {
            var groupIndex = groupsToMerge[i];
            tempGroup.concat(groups[groupIndex]);
        }
    }
...
for (var i = 0; i < mergedGroups.length; i++) {
    colorCode(mergedGroups[i], 'default', 'color background', 'none');
}
 ...
 */
function colorBackground(box, color, opacity=0.1) {
...
/* Function for mode of array taken verbatim from stack over flow
 * Needs to be updated and documented later
 */
function mode(array)
{
    if(array.length == 0)
